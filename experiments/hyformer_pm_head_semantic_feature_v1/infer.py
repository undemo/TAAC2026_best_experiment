"""PCVRHyFormer-PMHead-TimeToken-NSGroup inference script (uploaded by the contestant into the
...
with this PMHead experiment's defaults).
...
_FALLBACK_MODEL_CFG = {
    ...
    'pm_feature_norm_enabled': True,
    'time_token_enabled': True,
    'time_token_dim': 0,
    'time_token_dropout': 0.01,
    'time_token_norm_enabled': True,
    'time_token_insert_position': 'ns_tokens',
    'time_bucket_vocab_size': 0,
    'time_gap_bucket_vocab_size': 0,
    'use_sample_time_features': True,
    'use_seq_recency_features': True,
    'use_seq_time_summary': True,
    'time_token_ablation_mode': 'normal',
    'use_time_of_day_features': True,
    'time_tz_offset_hours': 8,
    'use_hour_embedding': False,
    'time_daypart_vocab_size': 7,
    'missing_aware_enabled': True,
    'sparse_missing_indicator_enabled': True,
    'missing_indicator_project_to_group': True,
    'missing_residual_alpha_init': 0.1,
    'missing_residual_alpha_learnable': True,
    'dense_missing_aware_enabled': True,
    'dense_stat_transform': 'signed_log1p',
    'dense_grouped_encoder_enabled': True,
    'dense_embedding_like_fids': [61, 87, 89, 90, 91],
    'dense_stat_like_fids': [62, 63, 64, 65, 66],
    'dense_missing_indicator_enabled': True,
    'dense_value_clip_abs': 0.0,
    'dense_encoder_dropout': 0.01,
}
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logging.info(
        "Resolved PM head cfg: enabled=%s dim=%s dropout=%s norm_enabled=%s",
        cfg.get('pm_head_enabled'),
        cfg.get('pm_feature_dim'),
        cfg.get('pm_feature_dropout'),
        cfg.get('pm_feature_norm_enabled'),
    )
    cfg['dense_embedding_like_fids'] = _parse_int_list(cfg.get('dense_embedding_like_fids'))
    cfg['dense_stat_like_fids'] = _parse_int_list(cfg.get('dense_stat_like_fids'))
    return cfg
...
def main() -> None:
    ...
    if not ns_groups_json:
        ns_groups_json = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            'ns_groups_h_semantic_feature_v1.json',
        )
    if ns_groups_json:
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            logits, _ = model.predict(model_input, eval_pm_mode="normal")
