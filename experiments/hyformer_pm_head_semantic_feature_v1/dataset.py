...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        zero_is_missing_fids: Optional[List[int]] = None,
    ) -> None:
        """
        Args:
        ...
        self.zero_is_missing_fids = set(int(fid) for fid in (zero_is_missing_fids or []))
        ...
        self._buf_user_int_missing = np.zeros((B, len(self.user_int_schema.entries)), dtype=np.float32)
        self._buf_user_int_zero = np.zeros((B, len(self.user_int_schema.entries)), dtype=np.float32)
        self._buf_user_int_minus1 = np.zeros((B, len(self.user_int_schema.entries)), dtype=np.float32)
        self._buf_item_int_missing = np.zeros((B, len(self.item_int_schema.entries)), dtype=np.float32)
        self._buf_item_int_zero = np.zeros((B, len(self.item_int_schema.entries)), dtype=np.float32)
        self._buf_item_int_minus1 = np.zeros((B, len(self.item_int_schema.entries)), dtype=np.float32)
        self._buf_user_dense_missing = np.zeros((B, len(self.user_dense_schema.entries)), dtype=np.float32)
        self._buf_user_dense_zero = np.zeros((B, len(self.user_dense_schema.entries)), dtype=np.float32)
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_lens = {}
        ...
        offset = 0
        for fid_idx, (fid, vs, dim) in enumerate(self._user_int_cols):
            ci = self._col_idx.get(f'user_int_feats_{fid}')
            self._user_int_plan.append((ci, dim, offset, vs, fid, fid_idx))
            offset += dim

        self._item_int_plan = []
        offset = 0
        for fid_idx, (fid, vs, dim) in enumerate(self._item_int_cols):
            ci = self._col_idx.get(f'item_int_feats_{fid}')
            self._item_int_plan.append((ci, dim, offset, vs, fid, fid_idx))
            offset += dim

        self._user_dense_plan = []
        offset = 0
        for fid_idx, (fid, dim) in enumerate(self._user_dense_cols):
            ci = self._col_idx.get(f'user_dense_feats_{fid}')
            self._user_dense_plan.append((ci, dim, offset, fid, fid_idx))
            offset += dim
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        if schema_path and os.path.exists(schema_path):
            ...
            logging.warning(
                "schema.json not found at %s; inferring feature schema from parquet columns. "
                "This path is intended for schema-less local data and logs inferred vocab/dim metadata.",
                schema_path,
            )
            raw = self._infer_schema_from_parquet()
        self.raw_schema: Dict[str, Any] = raw
    ...
    def _infer_schema_from_parquet(self) -> Dict[str, Any]:
        """Infer the official-style schema from flat parquet columns.

        The inferred vocab size is max(value)+1 over the available parquet
        files so existing dataset OOB checks remain valid.
        """
        first_schema = pq.ParquetFile(self._parquet_files[0]).schema_arrow
        names = first_schema.names

        def _fid_columns(prefix: str) -> List[Tuple[int, str]]:
            found = []
        ...
        def _max_and_dim(column_name: str, is_float: bool = False) -> Tuple[int, int]:
            max_value = 0
            max_dim = 1
            for path in self._parquet_files:
                pf = pq.ParquetFile(path)
                for rg_idx in range(pf.metadata.num_row_groups):
                    arr = pf.read_row_group(rg_idx, columns=[column_name]).column(0)
                    py_values = arr.to_pylist()
                    if pa.types.is_list(arr.type) or pa.types.is_large_list(arr.type):
                        for value in py_values:
                            if value is None:
                                continue
                            max_dim = max(max_dim, len(value))
                            for v in value:
                                if v is None:
                                    continue
                                try:
                        ...
                        for v in py_values:
                            ...
                            try:
                                ...
                                continue
            return max(max_value + 1, 1), max_dim

        user_int = []
        for fid, col in _fid_columns("user_int_feats"):
            vs, dim = _max_and_dim(col, is_float=False)
            user_int.append([fid, vs, dim])

        item_int = []
        for fid, col in _fid_columns("item_int_feats"):
            vs, dim = _max_and_dim(col, is_float=False)
            item_int.append([fid, vs, dim])

        user_dense = []
        for fid, col in _fid_columns("user_dense_feats"):
            _, dim = _max_and_dim(col, is_float=True)
            user_dense.append([fid, dim])

        seq_ts_fids = {"seq_a": 38, "seq_b": 67, "seq_c": 27, "seq_d": None}
        seq_prefixes = {
            "seq_a": "domain_a_seq",
            "seq_b": "domain_b_seq",
            "seq_c": "domain_c_seq",
        ...
        seq: Dict[str, Any] = {}
        for domain, prefix in seq_prefixes.items():
            features = []
            for fid, col in _fid_columns(prefix):
                vs, _ = _max_and_dim(col, is_float=False)
                features.append([fid, vs])
            if features:
                seq[domain] = {
                    "prefix": prefix,
                    "ts_fid": seq_ts_fids[domain],
                    "features": features,
                }

        logging.info(
            "Inferred schema: user_int=%d item_int=%d user_dense=%d seq_domains=%s",
            len(user_int), len(item_int), len(user_dense), sorted(seq.keys()),
        )
        return {"user_int": user_int, "item_int": item_int, "user_dense": user_dense, "seq": seq}
    ...
    def _pad_varlen_int_column(
        ...
        max_len: int,
        B: int,
    ) -> Tuple["npt.NDArray[np.int64]", "npt.NDArray[np.int64]", "npt.NDArray[np.bool_]", "npt.NDArray[np.bool_]", "npt.NDArray[np.bool_]"]:
        ...
        zero = np.zeros(B, dtype=bool)
        minus1 = np.zeros(B, dtype=bool)
        null_mask = arrow_col.is_null().to_numpy(zero_copy_only=False).astype(bool)

        for i in range(B):
            start, end = int(offsets[i]), int(offsets[i + 1])
            raw_len = end - start
            if null_mask[i] or raw_len <= 0:
                missing[i] = True
                continue
            use_len = min(raw_len, max_len)
            raw_vals = values[start:start + use_len]
            zero[i] = bool((raw_vals == 0).any())
            minus1[i] = bool((raw_vals == -1).any())
            valid = raw_vals != -1
            missing[i] = not bool(valid.any())
            padded[i, :use_len] = raw_vals
            lengths[i] = use_len

        padded[padded <= 0] = 0
        return padded, lengths, missing, zero, minus1
    ...
    def _pad_varlen_float_column(
        ...
        max_dim: int,
        B: int,
    ) -> Tuple["npt.NDArray[np.float32]", "npt.NDArray[np.bool_]", "npt.NDArray[np.bool_]"]:
        ...
        for i in range(B):
            ...
            raw_vals = values[start:start + use_len].astype(np.float32)
            finite = np.isfinite(raw_vals)
            missing[i] = not bool(finite.any())
            zero[i] = bool((raw_vals[finite] == 0).any()) if finite.any() else False
            raw_vals[~finite] = 0.0
            padded[i, :use_len] = raw_vals

        return padded, missing, zero
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        user_int_zero = self._buf_user_int_zero[:B]
        user_int_minus1 = self._buf_user_int_minus1[:B]
        user_int[:] = 0
        user_int_missing[:] = 0
        user_int_zero[:] = 0
        user_int_minus1[:] = 0
        for ci, dim, offset, vs, fid, fid_idx in self._user_int_plan:
            col = batch.column(ci)
            if dim == 1:
                null_mask = col.is_null().to_numpy(zero_copy_only=False).astype(bool)
                arr = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
                zero_mask = arr == 0
                minus1_mask = arr == -1
                missing_mask = null_mask | minus1_mask
                if fid in self.zero_is_missing_fids:
                    missing_mask = missing_mask | zero_mask
                user_int_missing[:, fid_idx] = missing_mask.astype(np.float32)
                user_int_zero[:, fid_idx] = zero_mask.astype(np.float32)
                user_int_minus1[:, fid_idx] = minus1_mask.astype(np.float32)
                arr[arr <= 0] = 0
                if vs > 0:
                ...
                padded, _, missing_mask, zero_mask, minus1_mask = self._pad_varlen_int_column(col, dim, B)
                if fid in self.zero_is_missing_fids:
                    missing_mask = missing_mask | zero_mask
                user_int_missing[:, fid_idx] = missing_mask.astype(np.float32)
                user_int_zero[:, fid_idx] = zero_mask.astype(np.float32)
        ...
        item_int_zero = self._buf_item_int_zero[:B]
        item_int_minus1 = self._buf_item_int_minus1[:B]
        item_int[:] = 0
        item_int_missing[:] = 0
        item_int_zero[:] = 0
        item_int_minus1[:] = 0
        for ci, dim, offset, vs, fid, fid_idx in self._item_int_plan:
            col = batch.column(ci)
            if dim == 1:
                null_mask = col.is_null().to_numpy(zero_copy_only=False).astype(bool)
                arr = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
                ...
                if fid in self.zero_is_missing_fids:
                    missing_mask = missing_mask | zero_mask
                item_int_missing[:, fid_idx] = missing_mask.astype(np.float32)
                item_int_zero[:, fid_idx] = zero_mask.astype(np.float32)
                item_int_minus1[:, fid_idx] = minus1_mask.astype(np.float32)
                arr[arr <= 0] = 0
                if vs > 0:
        ...
        user_dense_zero = self._buf_user_dense_zero[:B]
        user_dense[:] = 0
        user_dense_missing[:] = 0
        user_dense_zero[:] = 0
        for ci, dim, offset, fid, fid_idx in self._user_dense_plan:
            col = batch.column(ci)
            padded, missing_mask, zero_mask = self._pad_varlen_float_column(col, dim, B)
            user_dense_missing[:, fid_idx] = missing_mask.astype(np.float32)
            user_dense_zero[:, fid_idx] = zero_mask.astype(np.float32)
        ...
        result = {
            ...
            'user_int_missing_mask': torch.from_numpy(user_int_missing.copy()),
            'user_int_zero_mask': torch.from_numpy(user_int_zero.copy()),
            'user_int_minus1_mask': torch.from_numpy(user_int_minus1.copy()),
            'item_int_missing_mask': torch.from_numpy(item_int_missing.copy()),
            'item_int_zero_mask': torch.from_numpy(item_int_zero.copy()),
            'item_int_minus1_mask': torch.from_numpy(item_int_minus1.copy()),
            'user_dense_missing_mask': torch.from_numpy(user_dense_missing.copy()),
            'user_dense_zero_mask': torch.from_numpy(user_dense_zero.copy()),
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    valid_start_rg = n_train_rgs if n_train_rgs < total_rgs else max(0, total_rgs - n_valid_rgs)
    ...
    train_dataset = PCVRParquetDataset(
        ...
        zero_is_missing_fids=zero_is_missing_fids,
    )
    ...
    train_seen_user_ids = set()
    train_seen_item_ids = set()
    if collect_seen_ids:
        train_seen_user_ids = _collect_seen_ids(rg_info[:n_train_rgs], 'user_id')
        train_seen_item_ids = _collect_seen_ids(rg_info[:n_train_rgs], 'item_id')
    train_dataset.train_seen_user_ids = train_seen_user_ids
    train_dataset.train_seen_item_ids = train_seen_item_ids
    train_dataset.valid_row_group_range = (valid_start_rg, total_rgs)
    logging.info(
        "Cold diagnostics seen-set collection: enabled=%s, users=%d, items=%d.",
        collect_seen_ids,
        len(train_seen_user_ids),
        len(train_seen_item_ids),
    )

    return train_loader, valid_loader, train_dataset


def _normalize_id_for_seen(value: Any) -> Optional[int]:
    """Normalize user/item ids for cold-start diagnostics.

    Missing and non-positive ids are ignored so they are treated as cold at
    validation time instead of making a synthetic zero-id warm.
    """
    if value is None:
        return None
    try:
...
def _collect_seen_ids(
    ...
    """Collect ids from selected training Row Groups.

    This deliberately uses a plain Python set for the first cold diagnostics
    implementation. It is simple and exact; if full-data cardinality becomes a
    bottleneck, this can be replaced by a hash set on disk or a Bloom filter.
    """
    seen = set()
    if not rg_info:
        logging.warning(
            "Cold diagnostics seen set for %s is empty because no training Row Groups were selected.",
            column_name,
        )
        return seen
    ...
    for file_path, rg_idx, _ in rg_info:
        by_file.setdefault(file_path, []).append(rg_idx)

    for file_path, row_groups in by_file.items():
        pf = pq.ParquetFile(file_path)
        if column_name not in pf.schema_arrow.names:
            logging.warning(
                "Cold diagnostics column %s is missing from %s; seen set will be incomplete.",
                column_name,
                file_path,
            )
            continue
        for rg_idx in row_groups:
            ...
            for value in table.column(column_name).to_pylist():
                norm = _normalize_id_for_seen(value)
                if norm is not None:
                    seen.add(norm)
    return seen
